...
func loadEnv() []string {
        ...
        if err != nil {
                log.Println("Не удалось загрузить .env, используем стандартные сервисы")
                return []string{"sshd", "nginx"} // Значения по умолчанию
        }

        services := os.Getenv("SERVICES") // Получаем список сервисов
        if services == "" {
                log.Println("Переменная SERVICES пуста, используем стандартные сервисы")
                return []string{"sshd", "nginx"}
        }
